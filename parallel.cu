...
__global__ void count_node_to_node_intersections(int *counts, bool *batch, int num_rows, int num_nodes)
{
    ...
    int tx = threadIdx.x;
    int tid = bx + tx;
    int row = tid / (num_nodes * num_nodes);
    int node_y = (tid / num_nodes) % num_nodes;
    int node_z = tid % num_nodes;
    int row_tile = tx / (num_nodes);

    __shared__ bool batchTile[BLOCK_SIZE];
    if (row < num_rows) {
        batchTile[tx] = batch[row * num_nodes + node_z];
    } else {
        batchTile[tx] = false;
    }
    __syncthreads();
    ...
    if (row < num_rows) {
        if (batchTile[row_tile * num_nodes + node_y] && batchTile[row_tile * num_nodes + node_z]) {
